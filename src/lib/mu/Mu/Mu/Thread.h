...
class Thread
{
  public:
    ...
    struct BackTraceFrame
    {
        BackTraceFrame(Node* n=0, const Symbol* s=0) 
            : node(n), symbol(s), filename(0), linenum(0), charnum(0) {}
    ...
    const Value         call(const Function*,
    ...
    struct JumpRecord
    {
        JumpRecord(Thread& t, unsigned int flags) 
            : thread(t) { t.jumpPointBegin(flags); }
  ...
  private:
    ...
    void		popStackFrame(size_t offset, size_t size)
                        { _stackOffset = offset; _stack.resize(size); }
...
inline void 
Thread::StackRecord::beginActivation(size_t s) 
{ 
    _thread->beginActivation(s); 
}

inline void 
Thread::StackRecord::newStackFrame(size_t s)
{
    _thread->newStackFrame(s);
}

inline void 
Thread::StackRecord::endParameters() 
{ 
...
inline void
Thread::StackRecord::popStackFrame()
{
