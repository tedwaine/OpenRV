...
#endif
    pack.group->debug("thread_main: pthread_setspecific: %p %p %p %p", pack.group->_funcKey, (void *) pack.func, pack.group->_dataKey, pack.data);
...
thread_group::add_thread(int num_to_add, size_t stack_mult, const func_vector* funcs, const data_vector* datas)
{
    ...
    if (num_to_add)
    {
        debug("add_thread: resizing _threads and _packages to %d + %d, _default_stack_size: %d, stack_mult: %d", _threads.size(), num_to_add, _default_stack_size, stack_mult);
...
thread_group::control_wait(bool allThreads, double seconds)
{
	...
	lock(_wait_mutex);
        ...
        if (allThreads)
        {
            ...
            {
                debug("control_wait: control waiting on %d threads, timeout %g seconds", 
                ...
                if (seconds)
                {
                    ...
                    if (timedout) 
                    {
                        debug("control_wait: control wait timed out, %d threads remaining, _wait_cond and _wait_mutex", (_num_threads - _num_finished));
                        break;
                    }
                }
                else
                {
                    ...
                    debug("control_wait: control wait_cond returned, _wait_cond and _wait_mutex");
                }
            }
        }
        else
        {
            ...
            {
                debug("control_wait: control waiting on any thread, timeout %g seconds, _wait_cond and _wait_mutex", seconds);
                ...
                if (seconds)
                {
                    ...
                    if (timedout) 
                    {
                        debug("control_wait: control wait on any thread timed out");
                        break;
                    }
                }
                else
...
thread_group::worker_wait()
{
    ...
    while (_wait_flag)
    {
        debug("worker_wait: worker waiting on _worker_cond with _worker_mutex");
...
thread_group::worker_jump()
{
    ...
    else
    {
        ...
        debug("worker_jump: worker decremented, num_threads is now %d. _control_mutex locked", _num_threads);
...
thread_group::worker_cleanup()
{
    ...
    _num_threads--;
    _join_thread = pthread_self();
    debug("worker_cleanup: worker decremented, num_threads is now %d, _control_mutex locked", _num_threads);
...
thread_group::dispatch(thread_group::thread_function func, void* data)
{
    ...
    {
	debug("dispatch: control: thread_group: can't dispatch: no idle workers");
	//
