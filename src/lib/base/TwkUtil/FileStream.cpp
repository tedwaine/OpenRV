...
class MbpsCalculator
{
  public:
    ...
    void resetMbps() { m_totalBytes = 0; m_totalTime = 0.0;  };
...
MbpsCalculator::Monitor::~Monitor()
{
    m_calc.update (m_bytes);
}
...
void
MbpsCalculator::start()
{
    pthread_mutex_lock (&m_mutex);

    if (0 == m_activeCount) m_timer.start();
    ++m_activeCount;
...
void
MbpsCalculator::update (size_t bytes)
{
    pthread_mutex_lock (&m_mutex);
...
FileStream::mbps()
{
    return mbpsCalc.mbps();
}

void
FileStream::resetMbps()
{
    mbpsCalc.resetMbps();
}
...
KernelReadRequest::KernelReadRequest()
{
    clear();
}

void
KernelReadRequest::clear()
{
    //
...
KernelReadRequest *
KernelReadRequestList::nextFreeRequest()
{
...
void
KernelReadRequestList::handleCompleted (
    ...
    long errorCode)
{
    ...
    if (req->u.c.nbytes != bytesRead)
    {
        TWK_THROW_EXC_STREAM ("KernelReadRequestList async read incomplete (" << 
                bytesRead << " bytes out of " << req->u.c.nbytes << ")");
    }
...
KernelReadRequestList::update()
{
    ...
    //  Launch requests
    //
    int n = io_submit (context, 
            currentRequests.size(), (struct iocb **) &(currentRequests[0]));
    if (currentRequests.size() != n)
    {
        TWK_THROW_EXC_STREAM ("io_submit returned " << n << 
                ", expected " << currentRequests.size() << ". ASync IO unavailable");
    }
    requestsInFlight += n;
...
PosixReadRequest::PosixReadRequest()
{
    clear();
}

void
PosixReadRequest::clear()
{
    //
    //  Don't really know what all the fields here are, or what they
    //  do, so zero everything.  Also, there are private fields in
...
void
PosixReadRequestList::handleCompleted (PosixReadRequest *req)
{
    ...
    //
    {
        ...
        if (0 > ret)
        {
            TWK_THROW_EXC_STREAM ("aio_read failed (1), returned " << errno 
                    << ", " << strerror(errno) << ". Async IO unavailable");
        }
    }
    else
    //
...
PosixReadRequestList::update()
{
    for (RecVec::iterator i = requests.begin(); i != requests.end(); ++i)
    {
        if (i->inUse())
        {
            ...
            //  
            {
                TWK_THROW_EXC_STREAM ("aio_error failed, returned " << errno 
                        << ", " << strerror(errno) << ". ASync IO unavailable");
            }
        }
        ...
        //
        {
            ...
            if (0 > ret)
            {
                ...
                {
                    TWK_THROW_EXC_STREAM ("aio_read failed (2), returned " << errno 
                            << ", " << strerror(errno) << ". ASync IO unavailable");
                }
            }
            else
    ...
    */
    ...
    if (m_file == -1)
    {
        if (direct) 
        {
            ...
            {
                cerr << "WARNING: filesystem does not support direct (unbuffered) reads, falling back to buffered reads." << endl;
            }
            direct = 0;
            m_type = Buffering;
    ...
    if (m_readSize) m_fileSize = std::min(m_readSize, size_t(m_fileSize));

    /*
    fprintf (stderr, "%p FileStream type %d request for %d byte file\n", pthread_self(), m_type, m_fileSize);
    fflush (stderr);
    */
    ...
    if (m_type == ASyncNonBuffering)
    {
        ...
        if (!m_rawdata) TWK_THROW_EXC_STREAM ("Out of memory");
...
void 
FileStream::deleteDataPointer(void* p)
{
    MemPool::dealloc(p);
}
...
#define Copy64(Dest, Src) \
         *( (PULONGLONG) &(Dest)) = *( (PULONGLONG) &(Src)) 
...
FileStream::initialize()
{
    ...
    //  cerr << "m_type " << m_type << " asyncnonbuffering " << ASyncNonBuffering << endl;
    ...
    {
        cout << "THROWING" << endl;
        TWK_THROW_EXC_STREAM("CreateFile: cannot open " << m_filename <<
                " error: " << GetLastError());
    }
    ...
    {
        imp->m_map = CreateFileMapping(imp->m_file,
                                         NULL,
                                         PAGE_READONLY,
                                         0,
                                         0,
    ...
    else if (m_type == ASyncBuffering || m_type == ASyncNonBuffering)
    {
        ...
        if (imp->m_doneEvent == INVALID_HANDLE_VALUE) abort();
        ...
        for (size_t i=0; i < 8; i++)
        {
            ...
            if (remaining > 0)
            {
                //  cerr << "calling ReadFileEx i " << i << " remaining " << remaining << endl;
                if (!ReadFileEx(imp->m_file, 
                                readLocation,
                                imp->m_requestSize, 
                                o,
                                completeFunc))
                {
                    TWK_THROW_EXC_STREAM("CreateFile: cannot read " << m_filename 
                                         << " error: " << GetLastError());
                }
...
FileStream::~FileStream()
{
    ...
    if (imp->m_map)
    {
        if (m_rawdata) UnmapViewOfFile(m_rawdata);
        if (imp->m_map != INVALID_HANDLE_VALUE) CloseHandle(imp->m_map);
        if (imp->m_file && imp->m_file != INVALID_HANDLE_VALUE) CloseHandle(imp->m_file);
    }
    else if (m_deleteOnDestruction)
    {
        deleteDataPointer(m_rawdata);
