...
public:
    ...
    static void hms( double secondsTotal,
...
//
//  Timer instance you can put on the stack, but avoid even construction cost
//  if "debug" flag to constructor is not true.
//

class TWKUTIL_EXPORT DebugTimer 
{
  public:
    DebugTimer(bool debug, bool start=false) : m_timer(0)
    {
        if (debug) m_timer = new Timer(start);
    }
    ~DebugTimer() { if (m_timer) delete m_timer; }

    void   start()       { if (m_timer)       m_timer->start(); }
